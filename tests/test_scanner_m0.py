...
from catlogs.exporter import export_audit_report_pdf
...
class TestScannerM0(unittest.TestCase):
    ...
    def test_export_audit_report_pdf_generates_valid_file(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            pdf_path = os.path.join(tmpdir, "audit_report.pdf")
            findings = [{
                "rule_name": "embedded_execution_pattern",
                "severity": "high",
                "path": "/tmp/payload.ps1",
                "summary": "PowerShell payload detected",
                "details": "Contains PowerShell execution pattern.",
            }]
            ...
            self.assertIn(b"0 0 0 rg", raw)
            self.assertIn(b"CatLogs Audit Report", raw)
