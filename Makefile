...
.PHONY: all install run build clean distclean help package test unitest
...
unitest: test
	@echo "──▶ Running C static analysis & strict compilation..."
	@gcc -Wall -Wextra -Werror -g -O0 -o keylogger/keylogger_test keylogger/keylogger.c -lX11
	@if command -v cppcheck >/dev/null 2>&1; then \
		echo "    Running cppcheck..."; \
		cppcheck --enable=all --suppress=missingIncludeSystem keylogger/keylogger.c; \
	fi
	@echo "──▶ Running C memory leak and crash tests (Valgrind)..."
	@valgrind --leak-check=full --track-origins=yes --error-exitcode=1 ./keylogger/keylogger_test --version > /dev/null 2>&1
	@valgrind --leak-check=full --track-origins=yes --error-exitcode=1 ./keylogger/keylogger_test --help > /dev/null 2>&1
	@valgrind --leak-check=full --track-origins=yes --error-exitcode=1 ./keylogger/keylogger_test /tmp/testlog.txt > /dev/null 2>&1 & PID=$$!; sleep 2; kill -INT $$PID; wait $$PID
	@echo "──▶ Running AddressSanitizer (ASAN) and UndefinedBehaviorSanitizer (UBSAN)..."
	@gcc -fsanitize=address,undefined -g -O0 -o keylogger/keylogger_asan keylogger/keylogger.c -lX11
	@./keylogger/keylogger_asan --version > /dev/null 2>&1
	@./keylogger/keylogger_asan --help > /dev/null 2>&1
	@./keylogger/keylogger_asan /tmp/testlog.txt > /dev/null 2>&1 & PID=$$!; sleep 2; kill -INT $$PID; wait $$PID
	@rm -f keylogger/keylogger_test keylogger/keylogger_asan /tmp/testlog.txt /tmp/keylogger.pid
	@echo "──▶ All unit tests passed! No crashes, leaks, or undefined behavior detected."
...
help:
	...
	@echo "  make unitest     Run python tests and C memory leak/crash tests"
	@echo ""
