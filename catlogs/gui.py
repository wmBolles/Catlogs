...
from .export_log import log_export, load_export_history
from .read_errors import load_read_errors, clear_read_errors
from .log_parsers import LOG_FORMAT_CHOICES, parse_log_file
...
THEMES = {
    ...
    "solarized_light": {
        "bg": "#fdf6e3", "bg_secondary": "#eee8d5", "bg_table": "#fdf6e3",
        "fg": "#657b83", "fg_secondary": "#586e75", "accent": "#2aa198",
        "header_bg": "#eee8d5", "btn_bg": "#e8e2d0", "btn_fg": "#657b83",
        "border": "#ccc8b7", "row_even": "#fdf6e3", "row_odd": "#eee8d5",
        "select_bg": "#d6d2c4", "select_fg": "#000000", "nav_bg": "#fdf6e3", "nav_active": "#eee8d5", "accent_hover": "#268bd2", "fg_dim": "#93a1a1", "success": "#859900", "warning": "#b58900", "error": "#dc322f",
        "entry_bg": "#eee8d5", "detail_bg": "#fdf6e3"},
    "github_light": {
        "bg": "#ffffff", "bg_secondary": "#f6f8fa", "bg_table": "#ffffff",
        "fg": "#24292f", "fg_secondary": "#57606a", "accent": "#0969da",
        "header_bg": "#f6f8fa", "btn_bg": "#f3f4f6", "btn_fg": "#24292f",
        "border": "#d0d7de", "row_even": "#ffffff", "row_odd": "#f6f8fa",
        "select_bg": "#0969da", "select_fg": "#ffffff", "nav_bg": "#ffffff", "nav_active": "#f6f8fa", "accent_hover": "#0969da", "fg_dim": "#57606a", "success": "#2da44e", "warning": "#bf8700", "error": "#cf222e",
        "entry_bg": "#f6f8fa", "detail_bg": "#ffffff"},
    "material_light": {
        "bg": "#fafafa", "bg_secondary": "#f5f5f5", "bg_table": "#fafafa",
        "fg": "#546e7a", "fg_secondary": "#90a4ae", "accent": "#3949ab",
        "header_bg": "#e0e0e0", "btn_bg": "#eeeeee", "btn_fg": "#546e7a",
        "border": "#bdbdbd", "row_even": "#fafafa", "row_odd": "#f5f5f5",
        "select_bg": "#3949ab", "select_fg": "#ffffff", "nav_bg": "#fafafa", "nav_active": "#f5f5f5", "accent_hover": "#3949ab", "fg_dim": "#90a4ae", "success": "#43a047", "warning": "#fbc02d", "error": "#e53935",
        "entry_bg": "#f5f5f5", "detail_bg": "#fafafa"}
}
...
class CatLogsApp:
    ...
    def _apply_theme(self):
        ...
        style.map("TCheckbutton",
                  ...
                  indicatorbackground=[("pressed", c["select_bg"]), ("active", c["bg"])],
                  indicatorforeground=[("disabled", c["fg_dim"])])
    ...
    def _rebuild_nav(self):
        ...
        for page_id, label in pages:
            if page_id != "logs" and not enabled_features.get(page_id, True):
                continue
        ...
        if hasattr(self, '_current_page') and hasattr(self, 'logs_page'):
            target_page = self._current_page if self._current_page in self._nav_buttons else "logs"
...
Useful Resources & Documentation:"""
    ...
    def _refresh_processes(self):
        ...
        def worker():
            try:
                ...
                err_msg = f"Unable to read process list: {e}"
                self.root.after(0, lambda m=err_msg: self._apply_process_refresh_result(None, m))
                return
    ...
    def _show_settings(self):
        ...
        features_frame = ttk.Frame(content_container)
        ...
        def switch_tab(tab_name):
            ...
            btn_tab_features.configure(style="Nav.TButton")
            ...
            if tab_name == "paths":
                ...
                btn_tab_features.configure(style="NavActive.TButton")
        ...
        btn_tab_security.pack(side=tk.LEFT, padx=(0, 6))

        btn_tab_features = ttk.Button(tab_nav_frame, text="Features", style="Nav.TButton",
                                      command=lambda: switch_tab("features"))
        btn_tab_features.pack(side=tk.LEFT)
        ...
        ttk.Label(theme_card, text="Base Text Size (px):", font=(self.main_font, 11, "bold"),
                  background=self.colors["bg_secondary"]).pack(anchor=tk.W, pady=(16, 4))
        text_size_var = tk.StringVar(value=str(10 + getattr(self, "zoom_level", 0)))
        text_size_spin = ttk.Spinbox(theme_card, textvariable=text_size_var,
                                     from_=8, to=30, increment=1, width=10, font=(self.main_font, 11))
        text_size_spin.pack(anchor=tk.W)
        # -------------------------------------------------------------------
        # -------------------------------------------------------------------
        # TAB 3: SECURITY
        # -------------------------------------------------------------------
        ...
        # TAB 4: FEATURES
        # -------------------------------------------------------------------
        feat_header = ttk.Frame(features_frame)
        feat_header.pack(fill=tk.X, pady=(0, 16))

        ttk.Label(feat_header, text="Enable / Disable Features",
                  font=(self.main_font, 12, "bold")).pack(anchor=tk.W)
        ttk.Label(feat_header, text="Toggle which software features are visible in the navigation bar.",
                  font=(self.main_font, 9), foreground=self.colors["fg_dim"]).pack(anchor=tk.W, pady=(2, 0))

        feat_card = ttk.Frame(features_frame, style="Toolbar.TFrame", padding=20)
        feat_card.pack(fill=tk.BOTH, expand=True, pady=(0, 16))

        self.feature_vars = {}
        enabled_features = self.config.setdefault("enabled_features", {
            "processes": True,
            "scanner": True,
            "history": True,
            "read_errors": True,
        ...
        def on_feature_toggle(page_id, var):
            enabled_features[page_id] = var.get()

        for page_id, label in feature_labels.items():
            f_frame = ttk.Frame(feat_card, style="Toolbar.TFrame")
            f_frame.pack(fill=tk.X, anchor=tk.W, pady=2)
            var = tk.BooleanVar(value=enabled_features.get(page_id, True))
            self.feature_vars[page_id] = var
            cb = ttk.Checkbutton(f_frame, text=label, variable=var,
                                 command=lambda p=page_id, v=var: on_feature_toggle(p, v))
            cb.pack(side=tk.LEFT)
        ...
        def save_all_settings():
            ...
            try:
                new_text_size = int(text_size_var.get())
                new_zoom = new_text_size - 10
            except ValueError:
                new_zoom = getattr(self, "zoom_level", 0)
            ...
            if getattr(self, "zoom_level", 0) != new_zoom:
                self.zoom_level = new_zoom
                self.config["zoom_level"] = new_zoom
                app_changed = True

            # Save paths and features to config
    ...
    def _fetch_process_info(self, pid: int):
        ...
        try:
            ...
            def update_error():
                ...
                self.output_text.delete("1.0", tk.END)
                self.output_text.insert(
                    tk.END, f"  Warning: Could not fetch process info: {err_msg}\n", "error")
                self.output_text.configure(state=tk.DISABLED)
